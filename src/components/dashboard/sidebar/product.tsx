...
const Product = () => {
  ...
  const isProductRoute = pathname?.startsWith('/dashboard/product');

  return (
    <Collapsible className="group/collapsible" defaultOpen={isProductRoute}>
      <SidebarGroup>
        ...
        <CollapsibleContent>
          <SidebarGroupContent className="mt-2 ">
            <Link
              ...
              className={`flex items-center space-x-3  text-center  p-3 text-base  ${pathname === '/dashboard/product/all-products' && 'bg-[#201781] text-[#F87D3F]'}`}
            >
              <div className="w-[20px]" />
              <p
                className={`${pathname === '/dashboard/product/all-products' && 'text-[#F87D3F]'}`}
              >
                All Products
              </p>
            </Link>
