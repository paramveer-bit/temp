...
import { MagazineFormValues } from '@/utils/validation';
import { languages, magazineAdTypes, magazineCategories, printCirculations } from '@/utils/data';
import Dimensions from './dimensions';
import Circulation from './circulation';

const Information = ({ form }: { form: UseFormReturn<MagazineFormValues> }) => {
  return (
          ...
          render={({ field }) => (
            <FormItem className="w-full">
              ...
              <Select onValueChange={field.onChange} defaultValue={field.value}>
                <FormControl>
                  <SelectTrigger className="py-5 ">
                    <SelectValue placeholder="Select magazine type" className="" />
                  </SelectTrigger>
                </FormControl>
                <SelectContent className="max-h-[40vh]">
                  {magazineAdTypes.map((el) => (
                    <SelectItem value={el.value} key={el.value}>
                      {el.value}
                    </SelectItem>
                  ))}
